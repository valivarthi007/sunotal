...
export default function AllDevices() {
  ...
  const categories = ['Smartphones', 'Accessories', 'Tablets', 'Laptops'];
  const brands = ['Apple', 'Samsung', 'Xiaomi', 'OnePlus', 'Realme', 'Sony'];
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans bg-gray-50">
      ...
      <main className="flex-grow container mx-auto px-4 py-8">
        ...
        {/* Breadcrumbs & Title */}
        <div className="mb-8">
          <div className="text-sm text-gray-500 mb-2">Home / All Devices</div>
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <h1 className="text-3xl md:text-4xl font-black text-[#101828]">All Devices</h1>
                  ...
                  <CollapsibleContent className="space-y-3 pt-2">
                    {categories.map(cat => (
                      <div key={cat} className="flex items-center space-x-2">
                        <Checkbox id={`cat-${cat}`} />
                        <label htmlFor={`cat-${cat}`} className="text-sm font-medium leading-none">
                          {cat}
                        </label>
                      </div>
                    ))}
