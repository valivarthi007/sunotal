...
export default function Accessories() {
  ...
  const brands = ['Apple', 'Samsung', 'Sony', 'Boat', 'JBL'];
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans bg-gray-50">
      ...
      <main className="flex-grow container mx-auto px-4 py-8">
        ...
        {/* Breadcrumbs & Title */}
        <div className="mb-8">
          <div className="text-sm text-gray-500 mb-2">Home / All Devices / Accessories</div>
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <h1 className="text-3xl md:text-4xl font-black text-[#101828]">Accessories</h1>
            <p className="text-gray-500">{accessories.length} items found</p>
          </div>
        </div>
          ...
          <div className="w-full lg:w-3/4">
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-6 flex items-center justify-between">
              <span className="text-sm font-medium text-gray-500 hidden sm:inline-block">
                Showing 1-{accessories.length} of {accessories.length} products
              </span>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
              ...
              {accessories.map((product, index) => (
                <ProductCard key={`dup-${product.id}`} product={{...product, id: `dup-${product.id}`}} index={index+3} />
              ))}
            </div>
